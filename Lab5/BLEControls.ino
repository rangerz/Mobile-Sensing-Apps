...
#define BLE_DEVICE_NAME            "BLE Controls"
...
// LED status for Interrupt
static boolean all_on = false;
...
static void switchInterrupt() {
  if (all_on) {
    digitalWrite(DIGITAL_OUT_PIN, HIGH);
    analogWrite(PWM_PIN, 0xFF);
  } else {
    digitalWrite(DIGITAL_OUT_PIN, LOW);
    analogWrite(PWM_PIN, 0x00);
  }

  characteristic2_data[0] = (0x0C);
  characteristic2_data[1] = all_on ? (0x01) : (0x00);
  ble.sendNotify(character2_handle, characteristic2_data, CHARACTERISTIC2_MAX_LEN);
