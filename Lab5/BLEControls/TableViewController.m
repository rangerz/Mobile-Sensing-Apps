    ...
    for (int i = 0; i < length; i+=3)
    {
        ...
        else if (data[i] == 0x0B)
        {
            ...
            if (swAnalogIn.on) {
                ...
                // Value btw 0:4095 = > 0:1
                progresAI.progress = (float)Value/4095;
            } else {
                lblAnalogIn.text = @"----";
                progresAI.progress = 0.0;
            }
        }
        else if (data[i] == 0x0C)
        {
            bool all_on = data[i+1] == 0x01;
            swDigitalOut.on = all_on;
            sldPWM.value = (all_on) ? 0xFF : 0x00;
        }
    }
}
...
-(void) connectionTimer:(NSTimer *)timer
{
    ...
    NSString *myUUID = @"0C4C3C02-A282-F437-F6D4-7047EF77D10A";
    CBPeripheral *myPeripheral = NULL;

    for (int i = 0; i < ble.peripherals.count; i++) {
        CBPeripheral *p = [ble.peripherals objectAtIndex:i];

        if (NULL != p.identifier) {
            if ([p.identifier.UUIDString isEqualToString:myUUID]) {
                myPeripheral = p;
                break;
            }
        }
    ...
    if (myPeripheral) {
        [ble connectPeripheral: myPeripheral];
    } else {
        [btnConnect setTitle:@"Connect" forState:UIControlStateNormal];
        [indConnecting stopAnimating];
    }
